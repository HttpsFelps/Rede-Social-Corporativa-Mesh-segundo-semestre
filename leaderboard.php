...
if (isset($_SESSION['email_usuario'])) {
    ...
    if ($resultUsuario && mysql_num_rows($resultUsuario) > 0) {
        ...
        $queryVitorias = "SELECT vitorias FROM leaderboard WHERE nome = '$nomeUsuario'";
        $resultVitorias = mysql_query($queryVitorias);
        $vitoriasUsuario = ($resultVitorias && mysql_num_rows($resultVitorias) > 0) ? mysql_fetch_assoc($resultVitorias)['vitorias'] : 0;
    } else {
...
$query = "SELECT tb_contas.Nome, leaderboard.vitorias, tb_contas.Imagem 
          FROM tb_contas 
          INNER JOIN leaderboard ON tb_contas.Nome = leaderboard.nome 
          ORDER BY leaderboard.vitorias DESC";
        ...
        <ul class="leaderboard">
            <?php foreach ($ranking as $posicao => $usuario): ?>
                <li>
                    ...
                    ?>
                    <?= htmlspecialchars($usuario['Nome']) . " - " . $usuario['vitorias'] ?> vitórias
                </li>
            <?php endforeach; ?>
        </ul>
