...
if (isset($_POST['email']) && isset($_POST['action'])) {
    ...
    if ($action === 'follow') {
        ...
        $checkQuery = "SELECT * FROM seguindo WHERE seguidor_email = '$usuarioEmail' AND seguido_email = '$seguidoEmail'";
        $checkResult = mysql_query($checkQuery);

        if (mysql_num_rows($checkResult) === 0) {
            // Insere na tabela de seguindo
            $insertQuery = "INSERT INTO seguindo (seguidor_email, seguido_email) VALUES ('$usuarioEmail', '$seguidoEmail')";
        ...
        $deleteQuery = "DELETE FROM seguindo WHERE seguidor_email = '$usuarioEmail' AND seguido_email = '$seguidoEmail'";
