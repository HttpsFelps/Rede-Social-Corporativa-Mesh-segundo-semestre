...
$seguindoQuery = "SELECT seguido_email FROM seguindo WHERE seguidor_email = '$usuarioEmail'";
$seguindoResult = mysql_query($seguindoQuery);
$seguindoEmails = [];
while ($row = mysql_fetch_assoc($seguindoResult)) {
    $seguindoEmails[] = $row['seguido_email'];
}
?>
<br>
<div class="container">
        ...
        <ul>
            ...
            if (mysql_num_rows($result) > 0) {
                while ($row = mysql_fetch_assoc($result)) {
                    ...
                    $perfilLink = 'perfil.html?email=' . urlencode($row['Email']);
